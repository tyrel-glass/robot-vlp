...
# def convert_to_euler(pose_mat):
#     # Extract position (translation) from pose_mat
#     x = pose_mat[0][3]
#     y = pose_mat[1][3]
#     z = pose_mat[2][3]

#     # Extract yaw (Z rotation)
#     yaw = math.degrees(math.atan2(pose_mat[1][0], pose_mat[0][0]))  # atan2(r21, r11)

#     # Extract pitch (Y rotation)
#     sin_pitch = -pose_mat[2][0]  # r31
#     cos_pitch = math.sqrt(pose_mat[0][0] ** 2 + pose_mat[1][0] ** 2)  # sqrt(r11^2 + r21^2)
#     pitch = math.degrees(math.atan2(sin_pitch, cos_pitch))

#     # Extract roll (X rotation)
#     roll = math.degrees(math.atan2(pose_mat[2][1], pose_mat[2][2]))  # atan2(r32, r33)
...
class triad_openvr():
    def __init__(self, configfile_path=None):
        ...
        if configfile_path:
            ...
            # Iterate through the pose list to find the active devices and determine their type
            for i in range(openvr.k_erqerunMaxTrackedDeviceCount):
                if poses[i].bDeviceIsConnected:
